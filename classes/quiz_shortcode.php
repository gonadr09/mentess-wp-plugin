<?php
    ...
    class quiz_shortcode {
        ...
        public function get_questions_of_quiz($quiz_id) {
            ...
            // Obtener las preguntas junto con el tipo de respuesta y la informacion de la seccion �
            $query = $wpdb->prepare("
                SELECT 
                    q.question_id, 
                    q.question, 
                ...
                INNER JOIN {$wpdb->prefix}lg_responses_type rt ON q.response_type_id = rt.response_type_id
                WHERE s.quiz_id = %d
        ...
        function quiz_html_builder($quiz_id){
            ...
            // Verificar que no esta completada
            ...
            foreach ($sections_list as $key => $value) {
                ...
                // Obtener todas las preguntas de la seccion
                $html_questions = "";
        ...
        function save_form($post){
            ...
            if ($is_quiz_completed) {
                $this->send_new_quiz_completed_email($quiz_id, $user_quiz_id);
                $message = $this->show_results($quiz_id, $user_quiz_id);
                return $message;
            } else {
                // Obtener las preguntas sin responder agrupadas por seccion
                $sections_with_unanswered = array();
                foreach ($questions_list as $question) {
                    $user_response = $this->get_user_response($user_quiz_id, $question['question_id']);
                    if (!$user_response || empty($user_response['response_text'])) {
                        $section_id = $question['section_id'];
                        if (!isset($sections_with_unanswered[$section_id])) {
                            $sections_with_unanswered[$section_id] = array(
                                'section_name' => $question['section_name'],
                                'section_order' => $question['section_order'],
                                'questions' => array()
                            );
                        }
                        $sections_with_unanswered[$section_id]['questions'][] = $question['order'] . ") " . $question['question'];
                    }
                }

                // Ordenar las secciones por el campo order
                uasort($sections_with_unanswered, function($a, $b) {
                    return $a['section_order'] - $b['section_order'];
                });

                // Generar el HTML del acordeon
                $unanswered_questions_html = '';
                foreach ($sections_with_unanswered as $section_id => $section) {
                    $questions_list = implode('<br>', $section['questions']);
                    $unanswered_questions_html .= '
                        <div class="mb-3">
                            <h5 class="text-primary">' . $section['section_name'] . '</h5>
                            <small>' . $questions_list . '</small>
                        </div>';
                }
                ...
                <div class="lg-container mt-4">
                    <div id="message" class="alert alert-primary mb-0" role="alert">
                        ...
                        <button class="btn btn-sm btn-outline-primary ms-2" type="button" data-bs-toggle="collapse" data-bs-target="#unansweredQuestionsAccordion">
                            Ver las preguntas que me faltan
                        </button>
                    </div>
                    <div class="collapse mt-3" id="unansweredQuestionsAccordion">
        ...
        function show_results($quiz_id, $user_quiz_id) {            
            ...
            foreach ($sections_list as $section) {
                // Comprobar si es una seccion general (sin puntajes) o una seccion con puntajes
        ...
        function write_end_doc() {
            ...
            // Recupera el ID del logo desde la personalizacion del tema
            ...
            if ($logo_id) {
                ...
                $logo_url = ''; // Si no hay logo configurado, dejamos la URL vacia
            }
		...
		<p>Gracias por confiar en Mentess para explorar tu camino profesional. Este informe combina tus motivaciones, estilos y atributos, y perfiles, reflejando lo que te apasiona, en qué eres realmente bueno/a y las características únicas de tu personalidad.<br><br>
                ...
                <div data-pdf="signature" class="d-flex flex-column gap-2 mt-5 mb-5" style="inline-size: fit-content; justify-self: center;">
                    ...
                    <div id="instragram-row" class="d-flex justify-content-center gap-1">
                        <svg width="'. $svg_width .'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.--><path d="M224.1 141c-63.6 0-114.9 51.3-114.9 114.9s51.3 114.9 114.9 114.9S339 319.5 339 255.9 287.7 141 224.1 141zm0 189.6c-41.1 0-74.7-33.5-74.7-74.7s33.5-74.7 74.7-74.7 74.7 33.5 74.7 74.7-33.6 74.7-74.7 74.7zm146.4-194.3c0 14.9-12 26.8-26.8 26.8-14.9 0-26.8-12-26.8-26.8s12-26.8 26.8-26.8 26.8 12 26.8 26.8zm76.1 27.2c-1.7-35.9-9.9-67.7-36.2-93.9-26.2-26.2-58-34.4-93.9-36.2-37-2.1-147.9-2.1-184.9 0-35.8 1.7-67.6 9.9-93.9 36.1s-34.4 58-36.2 93.9c-2.1 37-2.1 147.9 0 184.9 1.7 35.9 9.9 67.7 36.2 93.9s34.4 58 36.2 93.9c2.1 37 2.1 147.9 0 184.9 1.7 35.9 9.9 67.7 36.2 93.9 36.2c37 2.1 147.9 2.1 184.9 0 35.9-1.7 67.7-9.9 93.9-36.2 26.2-26.2 34.4-58 36.2-93.9 2.1-37 2.1-147.8 0-184.8zM398.8 388c-7.8 19.6-22.9 34.7-42.6 42.6-29.5 11.7-99.5 9-132.1 9s-102.7 2.6-132.1-9c-19.6-7.8-34.7-22.9-42.6-42.6-11.7-29.5-9-99.5-9-132.1s-2.6-102.7 9-132.1c7.8-19.6 22.9-34.7 42.6-42.6 29.5-11.7 99.5-9 132.1-9s102.7-2.6 132.1 9c19.6 7.8 34.7 22.9 42.6 42.6 11.7 29.5 9 99.5 9 132.1s2.7 102.7-9 132.1z"/></svg>
                            ...
                            scales: {
                                y: {
                                    ticks: {
                                        ...
                                        autoSkip: false, // Evita que Chart.js oculte algunos labels automaticamente
                                        font: {
                                            size: 11 // Ajusta el tamaño de la fuente si es necesario
                                        },
